...
npm run studio:sync  # 카테고리를 Sanity Studio 스키마로 내보내기
```
...
## 7. CMS(Sanity) 연동 — 고객사가 직접 관리하게 만들기

**납품 후 우리가 손대지 않으려면 이걸 붙여야 합니다.** 안 붙이면 상품 하나
바꿀 때마다 우리에게 연락이 옵니다.

고객사가 고칠 수 있는 범위는 **제품·홈 배너·페이지(회사소개·개인정보처리방침)** 입니다.
세 가지가 한꺼번에 `CONTENT_SOURCE=sanity` 로 갈립니다. 상품만 CMS로 옮기고
회사소개를 마크다운에 두면 "주소가 바뀌었어요" 연락이 계속 옵니다.

| 하는 일 | 누가 | 어디 |
| --- | --- | --- |
| Sanity 프로젝트 생성 | **고객사 계정으로** | `sanity-studio/README.md` |
| 카테고리 동기화 | 우리 | `npm run studio:sync` |
| 기존 마크다운 이관 | 우리 | `npm run export:sanity` (아래 7-2) |
| Studio 배포 | 우리 | `cd sanity-studio && npm run deploy` |
| 환경변수 설정 | 우리 | Cloudflare 프로젝트 설정 |
| 저장 시 자동 재배포 연결 | 우리 | 웹훅 ↔ 배포 훅 |
| 사용설명서 전달 | 우리 | `docs/고객사-사용설명서.md` |

### 붙이기 전에 판단하세요

콘텐츠가 분기에 한두 번 바뀌는 고객사라면 마크다운 그대로가 낫습니다.
계정이 하나 늘고 관리 포인트가 생깁니다. 월 단위로 바뀌면 붙이세요.

### CMS를 붙여도 완전히 손을 떼는 건 아닙니다

- Sanity가 요금제를 바꾸거나 서비스를 접으면 영향을 받습니다
- 웹훅이 끊기면 "저장했는데 안 바뀌어요" 연락이 옵니다
- 카테고리를 새로 만들려면 결국 코드를 고쳐야 합니다

**계정을 전부 고객사 명의로 만드는 것**이 이 중 대부분을 막습니다.

---

## 7-1. 연동 구조 — 이미 구현되어 있습니다
...
Studio 설정은 **sanity-studio/README.md**, 스키마는 **sanity-studio/schemaTypes/** 아래에 있습니다.
...
└─ pages/
   ...
   └─ robots.txt.ts
sanity-studio/                 고객사 Studio (별도 프로젝트, 사이트 빌드에 포함 안 됨)
├─ sanity.config.js            좌측 메뉴 구성
├─ schemaTypes/                product / page / slide / table
│  └─ categories.generated.js  site.config.ts 에서 생성 (npm run studio:sync)
